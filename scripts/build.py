...
class Program {
    static string ParseVersion(string versionPy) {
        if (!File.Exists(versionPy)) {
            return null;
        }
        Match m = Regex.Match(File.ReadAllText(versionPy), "VERSION\\s*=\\s*\"([^\"]+)\"");
        return m.Success ? m.Groups[1].Value : null;
    }

    static void EnsureVersionedLayout(string productDir) {
        string currentFile = Path.Combine(productDir, "current.txt");
        if (File.Exists(currentFile)) {
            return;
        }
        string flatPython = Path.Combine(productDir, "python");
        string flatSrc = Path.Combine(productDir, "src");
        if (!Directory.Exists(flatPython) || !Directory.Exists(flatSrc)) {
            return;
        }
        string ver = ParseVersion(Path.Combine(flatSrc, "_version.py")) ?? "0.0.0";
        string versionsRoot = Path.Combine(productDir, "versions");
        string dest = Path.Combine(versionsRoot, ver);
        Directory.CreateDirectory(versionsRoot);
        if (Directory.Exists(dest)) {
            Directory.Delete(dest, true);
        }
        Directory.CreateDirectory(dest);
        Directory.Move(flatPython, Path.Combine(dest, "python"));
        Directory.Move(flatSrc, Path.Combine(dest, "src"));
        File.WriteAllText(currentFile, ver);
    }
    ...
    static int Main(string[] args) {
        ...
        try {
            EnsureVersionedLayout(dir);
        } catch (Exception) {
            // Keep going; flat python/src layout is still supported below.
        }

        string python;
        string script;
        string workDir;
        string currentFile = Path.Combine(dir, "current.txt");
        if (File.Exists(currentFile)) {
            string ver = File.ReadAllText(currentFile).Trim();
            string payload = Path.Combine(dir, "versions", ver);
            python = Path.Combine(payload, "python", "python.exe");
            script = Path.Combine(payload, "src", "main.py");
            workDir = payload;
        } else {
            python = Path.Combine(dir, "python", "python.exe");
            script = Path.Combine(dir, "src", "main.py");
            workDir = dir;
        }
...
def _write_run_bat(dest: Path):
    ...
    bat.write_text(
        ...
        '  "versions\\%VER%\\python\\python.exe" -u "versions\\%VER%\\src\\main.py" %*\r\n'
        ') else (\r\n'
...
def build_installer(*, refresh_portable: bool = True):
    ...
    dist_app = DIST_APP.resolve().as_posix()
    iss_content = f"""; Inno Setup script for VoiceInput
; Auto-generated by build.py
; Installs into the same versions/{{ver}}+current.txt layout as in-app updates.
...
{setup_icon}OutputDir={DIST.resolve().as_posix()}
OutputBaseFilename=VoiceInput-{{#MyAppVersion}}-setup
Compression=lzma2
...
[Files]
Source: "{dist_app}\\python\\*"; DestDir: "{{app}}\\versions\\{{#MyAppVersion}}\\python"; Flags: ignoreversion recursesubdirs createallsubdirs
Source: "{dist_app}\\src\\*"; DestDir: "{{app}}\\versions\\{{#MyAppVersion}}\\src"; Flags: ignoreversion recursesubdirs createallsubdirs
Source: "{dist_app}\\VoiceInput.exe"; DestDir: "{{app}}"; Flags: ignoreversion
Source: "{dist_app}\\run.bat"; DestDir: "{{app}}"; Flags: ignoreversion skipifsourcedoesntexist
...
begin
  ...
  DelTree(ExpandConstant('{{app}}\\VoiceInput'), True, True, True);
  DeleteFile(ExpandConstant('{{app}}\\VoiceInput.exe.new'));
  ...
  begin
    ...
    try
      repeat
        ...
        begin
          ChildPath := VersionsRoot + '\\' + FindRec.Name;
...
begin
  ...
  begin
    CurrentFile := ExpandConstant('{{app}}\\current.txt');
    ForceDirectories(ExtractFileDir(CurrentFile));
    SaveStringToFile(CurrentFile, '{{#MyAppVersion}}', False);
    CleanupLegacyLayout;
  end;
end;
