...
from core.fault_policy import FAULT_POLICIES, BalloonMode, FaultPolicy
...
class FaultClassificationTests(unittest.TestCase):
    def test_free_tier_403_is_api_remote(self):
        ...
        self.assertIn(FaultKind.CREDENTIAL, policy.clears)
        self.assertFalse(policy.block_recording_start)
    ...
    def test_401_invalid_key_is_credential(self):
        ...
        policy = FAULT_POLICIES[event.kind]
        self.assertTrue(policy.activate_self)
        self.assertTrue(policy.block_recording_start)
    ...
    def test_config_disk_uses_modal_not_balloon(self):
        policy = FAULT_POLICIES[FaultKind.CONFIG_DISK]
        self.assertEqual(policy.balloon_mode, BalloonMode.NONE)
        self.assertTrue(policy.block_recording_start)

    def test_config_busy_blocks_recording(self):
        policy = FAULT_POLICIES[FaultKind.CONFIG_BUSY]
        self.assertTrue(policy.block_recording_start)
